...
if __name__ == '__main__':
    # MODIFIED: New objective function F9 (Rastrigin) based on the image.
    ...
    # MODIFIED: Parameters updated according to the new image for F9
    dim = 20
    lb = [-5.12] * dim  # Range is now [-5.12, 5.12]
    ub = [5.12] * dim   # Range is now [-5.12, 5.12]
    num_agents = 50
    max_iter = 1000

    # ADDED: Define the theoretical f_min for comparison. For Rastrigin, f_min is always 0.
    THEORETICAL_F_MIN = 0

    # MODIFIED: Changed the function call to use the new F9 function
    best_solution, best_fitness = sca(shifted_rastrigin_f9, lb, ub, dim, num_agents, max_iter)
